...
final class NewsListViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        bindViewModel()
        setupConstraints()
        let firstCategory = categories[selectedIndex].rawValue
        viewModel.load(category: firstCategory)
...
extension NewsListViewController: UICollectionViewDataSource, UICollectionViewDelegate  {
    ...
    func collectionView(
        ...
        if collectionView == categoryCollection {
            ...
            let isBookmarked = bookmarks.contains { $0.url == news.url }
            ...
            cell.configure(
                ...
                if isBookmarked {
                    self.removeBookmarkUseCase.execute(news)
                } else {
                    self.saveBookmarkUseCase.execute(news)
                    
                }
                self.bookmarks = self.getBookmarksUseCase.execute()
